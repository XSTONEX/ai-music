 ...
 *     by default 0.025.
 */
export default function Clouder(params) {
    ...
    function redirectToUrlBasedOnId(id) {
        // 假设ID对应的URL是 'https://yourwebsite.com/page/' + id
    ...
    function parametrize(p) {
        ...
        callback = p.callback ? p.callback : function (id) { redirectToUrlBasedOnId(id); };
        fontSize = p.fontSize ? p.fontSize : 25;
        fontShift = typeof (p.fontShift) != "undefined" ? p.fontShift : fontSize / 2;
        colorMax = p.colorMax ? parseColor(p.colorMax) : parseColor("#085abd");
        colorMin = p.colorMin ? parseColor(p.colorMin) : parseColor("#6eb8ff");
